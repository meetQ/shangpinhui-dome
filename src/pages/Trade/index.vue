  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address) in addressInfo" :key="address.id">
        ...
        <p @click="changeDefalut(address,addressInfo)">
      ...
      <div class="detail">
        ...
        <ul class="list clearFix" v-for="(order) in orderInfo.detailArrayList" :key="order.skuId">
          <li>
            <img style="height:100px;width:100px" :src="order.imgUrl">
          </li>
          <li>
            <p>
              {{order.skuName}}</p>
...
<script>
  ...
  export default {
    ...
    methods: {
      //修改默认地址
      changeDefalut(address, addressInfo) {
        //全部的isDefault为0 
      ...
      async submitOrder() {
        ...
        let data = {
          ...
          "consigneeTel": this.userDefaultAddress.phone, //最终收件人的电话号码
          "deliveryAddress": this.userDefaultAddress.fullAddress, //地址
          "paymentWay": "ONLINE", //线上支付
        ...
        if (result.code == 200) {
          ...
          //跳转路由
          this.$router.push(`/pay?id=${this.orderId}`)
        } else {
          alert("失败")
        }
      }
